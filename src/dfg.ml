...
let expr_to_node (e : expr) (r : result) : node =
  ...
  | EInput (n) -> "input", NInput n


let rec com_to_nodes (c : com) (r : result) : result =
  match c with
  | CAssgn(var, expr) ->
    let _, n = expr_to_node expr r in
    let r' = insert r var (var, n) in
    r'.com_map := ComMap.add c (var, n) !(r'.com_map);
    r'
  | CIf(_, branch) ->
...
and print_node ((v, n) : node) : string =
  v ^ " = " ^ (print_node' n)
